...
class RideViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.admin_user = User.objects.create_superuser(
            username='admin', email='admin@example.com', password='adminpass', first_name='Admin', last_name='User', phone_number='1234567890'
        )
        self.user = User.objects.create_user(
            username='testuser', email='testuser@example.com', password='testpass', first_name='Test', last_name='User', phone_number='1234567890'
        )
    ...
    def test_filter_rides_by_rider_email(self):
        self.client.force_login(self.admin_user)
        response = self.client.get(reverse('ride-list'), {'rider__email': 'testuser@example.com'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_sort_rides_by_pickup_time(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(response.data['results'][0]['pickup_time'] < response.data['results'][1]['pickup_time'])
    ...
    def test_pagination(self):
        ...
        expected_first_page_rides = [self.ride1, self.ride3]  # Get expected rides for the first page
        first_page_serializer = RideSerializer(expected_first_page_rides, many=True)
        self.assertEqual(first_page_response.data['results'], first_page_serializer.data)
