...
class RideViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        prefetch = Prefetch('events', queryset=RideEvent.objects.filter(created_at__gte=last_24_hours), to_attr='todays_ride_events')
        
        return Ride.objects.select_related('rider', 'driver').prefetch_related(prefetch).all()
    ...
    def distance_sort(self, request):
        ...
        queryset = self.get_queryset().annotate(
            distance_from_gps_position=geodesic((gps_latitude, gps_longitude), (ride.pickup_latitude, ride.pickup_longitude)).km
